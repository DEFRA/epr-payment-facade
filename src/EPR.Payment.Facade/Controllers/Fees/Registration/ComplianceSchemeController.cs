...
{
    [ApiController]
    [Route("api/fees/compliance-scheme")]
    [ApiVersion(1)]
    public class ComplianceSchemeController : ControllerBase
    {
        ...
        [MapToApiVersion(1)]
        [SwaggerOperation(
            Summary = "Calculate compliance scheme fee.",
            Description = "Calculate compliance scheme fee based on provided parameters."
        )]
        ...
        public async Task<ActionResult<FeeResponse>> CalculateFee([FromQuery] RegistrationFeeRequest request)
        {
            ...
            {
                _logger.LogError(ex, "An error occurred while calculating compliance scheme fee.");
